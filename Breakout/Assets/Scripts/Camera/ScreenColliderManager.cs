...
public class ScreenColliderManager : MonoBehaviour
{
    ...
    void CreateScreenColliders()
    {
        ...
        top.transform.position = new Vector3((bottomLeftScreenPoint.x - topRightScreenPoint.x) / 2f, anchor.transform.position.y, 0f);
        ...
        top.GetComponent<BoxCollider2D>().sharedMaterial = bounceMat;
        left.GetComponent<BoxCollider2D>().sharedMaterial = bounceMat;
        right.GetComponent<BoxCollider2D>().sharedMaterial = bounceMat;
    }
}